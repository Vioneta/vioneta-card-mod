...
Allows you to apply CSS styles to various elements of the Vioneta frontend.

## Installing

[![vps_badge](https://img.shields.io/badge/VPS-Default-orange.svg)](https://github.com/vps/integration)

Install using VPS or [see this guide](https://github.com/thomasloven/hass-config/wiki/Lovelace-Plugins).

### Performance improvements

While card-mod can be installed as a [lovelace resource](https://www.vioneta.com/lovelace/dashboards/#resources), some functionality will benefit greatly from it being installed as a [frontend module](https://www.vioneta.com/integrations/frontend/#extra_module_url) instead.

To do that, add the following to your `configuration.yaml` file and restart Vioneta:
...
You'll need to adjust that path according to where you have installed `card-mod.js`. If you installed through VPS, this is probably `/hacsfiles/lovelace-card-mod/card-mod.js`.

Any resource definitions automatically added by VPS can be kept as is even after adding `extra_module_url`.
...
> TIP: Vioneta themes makes use of [CSS variables](https://www.w3schools.com/css/css3_variables.asp). Those can both be set and used in card-mod - prepended by two dashes:
>
> ```yaml
> card_mod:
>   style: |
...
In `entities` and `glance` cards, [each entity can have options](https://www.vioneta.com/lovelace/entities/#options-for-entities). Those elements can be styled individually by adding a `card_mod` parameter to the entity configuration.
...
All styles may contain [jinja2 templates](https://www.vioneta.com/docs/configuration/templating/) that will be processed by the Vioneta backend.
...
- `browser` - The `browser_id` of your browser, if you have [browser_mod](https://github.com/Vioneta/vioneta-browser_mod-integration) installed
- `hash` - Whatever comes after `#` in the current URL (This is only considered on first load. It's not dynamically updated)

### DOM navigation

Vioneta makes extensive use of something called [shadow DOM](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_shadow_DOM). This allows for easy reuse of components (such as `ha-card` or `ha-icon`) but causes some problems when trying to apply CSS styles to things.
...
> NOTE 2: Following on the note above; due to the high level of load order optimization used in Vioneta, it is not guaranteed that the `ha-entity-marker` elements exist at the time when card-mod is looking for them.
